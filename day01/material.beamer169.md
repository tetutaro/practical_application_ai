...
* **人間の仕事は、残念ながら絶対に無くなりません**
    * 幸か不幸か、私の仕事が無くなることは、ほぼ無いでしょう
    * AI（機械学習）には、人間が行うことが山程あるから
* 以下、業務にAIを用いる例え話をします
    * AIを用いるために、人間は何をしなければならないのか
    * AIを用いることで、人間は何が便利になるのか
...
## AIに必要なものと、AIで便利になること

* AI に必要なもの
    * 人間の知見・経験を変換した（大量の！）データ
        * AIはどんなデータが必要か分からない
        * 人間が作る必要がある
    * 正解データ
...
* 目的の明確化
    * 「AIをする」ことが目的ではなく、きちんとした業務目標がある
* AIを行える環境
    * 業務のIT化、データのデジタル化
* AIを有効活用できる風土（意識）の醸成
    * 定期的にデータをチェックし、判断する（データドリブン）
* AIの特性への理解
    * データを見てみないと成功するか分からない（下分析の必要性）
    * 短期間で効果が出るものではなく、長期的な視点が必要
    * 生成されたデータを再び学習し、変化に対応する必要
* AIの適切な導入方法
    * 人間のサポート的な役割から、徐々に自動化を進める
